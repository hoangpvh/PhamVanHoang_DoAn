                    ...
                    <div class="card-body">
                        <form action="">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-4">
                                    <div class="form-group">
                                        <input type="text" name="email" class="form-control mg-r-15"
                                            placeholder="Email">
                                    </div>
                                </div>
                                        ...
                                        @foreach ($comments as $comment)
                                            <tr>
                                                ...
                                                <td style="vertical-align: middle; width: 11%">
                                                    <button type="button"
                                                        class="btn btn-block {{ $classStatus[$comment->cm_status] }} btn-xs">{{ $status[$comment->cm_status] }}</button>
                                                </td>
                                                @if (Auth::user()->can(['full-quyen-quan-ly']))
                                                    <td style="vertical-align: middle; width: 17%" class="text-center">
                                                        <div class="btn-group">
                                                            ...
                                                            <ul class="dropdown-menu action-transaction" role="menu">
                                                                <li><a href="{{ route('comment.delete', $comment->id) }}"
                                                                        class="btn-confirm-delete"><i
                                                                            class="fa fa-trash"></i> Delete</a></li>
                                                                @foreach ($status as $key => $item)
                                                                    <li class="update_book_tour"
                                                                        url='{{ route('comment.update.status', ['status' => $key, 'id' => $comment->id]) }}'>
